...
if [ $# == 5 ]; then
  ...
  echo "Step $step: remove poor sample with call rate < 95%"  
  king -b ${inrawfile}.bed --bysample --prefix temp_raw >> $logfile
  awk 'NR>1' temp_rawbySample.txt | awk '$7 > 0.05' | awk '{print $1, $2, "MissingMoreThan5"}' >> $outsamplefile
  $PLINK --bfile $inrawfile --remove $outsamplefile --make-bed --out $rawfile --noweb >> $logfile
  nout=$(awk 'NR>1' $outsamplefile | wc -l)
  echo "# Samples to be removed with call rate < 95% is $nout"  
else
  echo "Usage: qcZ_gen.sh {nickname} {rawfile} {chip} {overall_title} {pre-specified remove list}"
  exit
fi
...
#$R CMD BATCH -"${nickname}${step}bySample.txt" ${scripts}/samplecallrateZ.R

$R CMD BATCH "--args ${nickname}${step}bySample.txt ${overall_title} ${chip}" ${cPATH}/samplecallrateZ_gen.R
...
#$R CMD BATCH -${snpqcfile} ${scripts}/snpqcZ.R

$R CMD BATCH "--args ${snpqcfile} ${overall_title} ${chip}" ${cPATH}/snpqcZ_gen.R
...
#$R CMD BATCH -${sampleqcfile} -${half_ysnps} ${scripts}/gender.R
...
#R CMD BATCH -"${nickname}${step}apc.ped" $scripts/projpca.R
...
#R CMD BATCH -"${nickname}${step}bpc.ped" $scripts/EURprojpca.R
